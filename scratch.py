...
def t_stat(X, y):
    """Calculates robust t-stats for whether the regression coefficients of ``y`` on ``X`` are zero.
...
def pdf(z, tol=1e-5):
    """Calculates the density function of the KVB t-distribution.
...
def cdf(z, tol=1e-5):
    """Calculates the cumulative density function of the KVB t-distribution.
...
def quantile(q, tol=1e-3):
    ...
    Returns:
        The :math:`z` such that :math:`cdf(z) = q`
    """
    if q == 1:
        return sp.inf
    elif q == 0:
